...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	NVT_LOG("nvt_ts_probe: fw_version=%s", fw_version);
	if(strstr(saved_command_line,"qcom,mdss_dsi_nt36672_tianma_fhd_video") != NULL) {
		nvt_pnl_id = PANEL_TIANMA;
		init_tp_fm_info(0, fw_version, "tianma");
	}
	else if(strstr(saved_command_line,"qcom,mdss_dsi_nt36672_jdi_fhd_video") != NULL) {
		nvt_pnl_id = PANEL_JDI;
		init_tp_fm_info(0, fw_version, "jdi");
	}
	else {
		nvt_pnl_id = PANEL_UNSUPPORTED;
		NVT_ERR("nvt mp proc init failed. unsupported mdss dsi panel.\n");
		goto err_init_NVT_ts;
	}
#if defined(CONFIG_FB)
