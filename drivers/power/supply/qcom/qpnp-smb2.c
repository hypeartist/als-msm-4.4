...
static int smb2_parse_dt(struct smb2 *chip)
{
	...
	if (!node) {
		qpnp_err("device tree node missing\n");
		return -EINVAL;
	}
	...
	if (hwc_check_global){
		qpnp_err("sunxing get global set fcc max 2.3A");
	...
	if (chip->dt.float_option < 0 || chip->dt.float_option > 4) {
		qpnp_err("qcom,float-option is out of range [0, 4]\n");
		return -EINVAL;
	}
	...
	if ((chip->dt.chg_inhibit_thr_mv < 0 ||
		...
		qpnp_err("qcom,chg-inhibit-threshold-mv is incorrect\n");
		return -EINVAL;
	}
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SDP_CURRENT_MAX:
		...
		break;
	default:
		qpnp_err("get prop %d is not supported in usb\n", psp);
		rc = -EINVAL;
		break;
	}
	if (rc < 0) {
		qpnp_debug("Couldn't get prop %d rc = %d\n", psp, rc);
		return -ENODATA;
	}
	return 0;
}
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_RERUN_APSD:
		...
		break;
	default:
		qpnp_err("set prop %d is not supported\n", psp);
		rc = -EINVAL;
		break;
	}
...
static int smb2_init_usb_psy(struct smb2 *chip)
{
	...
	if (IS_ERR(chg->usb_psy)) {
		qpnp_err("Couldn't register USB power supply\n");
...
static int smb2_init_usb_port_psy(struct smb2 *chip)
{
	...
	if (IS_ERR(chg->usb_port_psy)) {
		qpnp_err("Couldn't register USB pc_port power supply\n");
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		...
		break;
	default:
		qpnp_debug("get prop %d is not supported in usb-main\n", psp);
		rc = -EINVAL;
		break;
	}
	if (rc < 0) {
...
static int smb2_init_usb_main_psy(struct smb2 *chip)
{
	...
	if (IS_ERR(chg->usb_main_psy)) {
		qpnp_err("Couldn't register USB main power supply\n");
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		#ifdef XIAOMI_CHARGER_RUNIN
		qpnp_err("lct battery_capacity =%d\n", val->intval);
		#endif
		#ifdef CONFIG_CHARGER_RUNIN
		runin_work(chg, val);
		#endif
	...
	case POWER_SUPPLY_PROP_CHARGER_TEMP:
		...
		if (rc < 0)
			qpnp_err("Couldn't get usb present rc=%d\n", rc);
	...
	case POWER_SUPPLY_PROP_FCC_STEPPER_ENABLE:
		...
		break;
	default:
		qpnp_err("batt power supply prop %d not supported\n", psp);
		return -EINVAL;
	}
...
static int smb2_init_batt_psy(struct smb2 *chip)
{
	...
	if (IS_ERR(chg->batt_psy)) {
		qpnp_err("Couldn't register battery power supply\n");
...
static int smb2_init_vbus_regulator(struct smb2 *chip)
{
	...
	if (IS_ERR(chg->vbus_vreg->rdev)) {
		...
		if (rc != -EPROBE_DEFER)
			qpnp_err("Couldn't register VBUS regualtor rc=%d\n", rc);
	}
...
static int smb2_init_vconn_regulator(struct smb2 *chip)
{
	...
	if (IS_ERR(chg->vconn_vreg->rdev)) {
		...
		if (rc != -EPROBE_DEFER)
			qpnp_err("Couldn't register VCONN regualtor rc=%d\n", rc);
	}
...
static int smb2_config_wipower_input_power(struct smb2 *chip, int uw)
{
	...
	if (rc < 0) {
		qpnp_err("Couldn't configure dc_icl_pt_lv rc = %d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		qpnp_err("Couldn't configure dc_icl_pt_hv rc = %d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		qpnp_err("Couldn't configure dc_icl_div2_lv rc = %d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		qpnp_err("Couldn't configure dc_icl_div2_mid_lv rc = %d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		qpnp_err("Couldn't configure dc_icl_div2_mid_hv rc = %d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		qpnp_err("Couldn't configure dc_icl_div2_hv rc = %d\n", rc);
		return rc;
	}
...
static int smb2_init_hw(struct smb2 *chip)
{
	...
	if (rc < 0) {
		qpnp_err("Couldn't set otg soft start rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		qpnp_err("Couldn't set otg current limit rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		qpnp_err("Couldn't read APSD_RESULT_STATUS rc=%d\n", rc);
		return rc;
	}
	...
	if (smblib_icl_override(chg, false) < 0) {
		qpnp_err("Couldn't disable ICL override rc=%d\n", rc);
		return rc;
	}
	...
	rc = smblib_masked_write(chg, SNARL_BARK_BITE_WD_CFG_REG,
			...
			val);
	if (rc) {
		qpnp_err("Couldn't configue WD config rc=%d\n", rc);
		return rc;
	}
...
static int smb2_chg_config_init(struct smb2 *chip)
{
	...
	revid_dev_node = of_parse_phandle(chip->chg.dev->of_node,
					  "qcom,pmic-revid", 0);
	if (!revid_dev_node) {
		qpnp_err("Missing qcom,pmic-revid property\n");
		return -EINVAL;
	}
	...
	case PM660_SUBTYPE:
		...
		break;
	default:
		qpnp_err("PMIC subtype %d not supported\n",
				...
				struct device_node *node, const char *irq_name)
{
	...
	if (irq < 0) {
		qpnp_err("Couldn't get irq %s byname\n", irq_name);
		return irq;
	}

	irq_index = smb2_get_irq_index_byname(irq_name);
	if (irq_index < 0) {
		qpnp_err("%s is not a defined irq\n", irq_name);
		return irq_index;
	}
	...
	if (rc < 0) {
		qpnp_err("Couldn't request irq %d\n", irq);
		return rc;
	}
...
static void smb2_create_debugfs(struct smb2 *chip)
{
	...
	if (IS_ERR_OR_NULL(chip->dfs_root)) {
		qpnp_err("Couldn't create charger debugfs rc=%ld\n",
			(long)chip->dfs_root);
		return;
	}
	...
	if (IS_ERR_OR_NULL(file))
		qpnp_err("Couldn't create force_batt_psy_update file rc=%ld\n",
			(long)file);

	file = debugfs_create_file("force_usb_psy_update", S_IRUSR | S_IWUSR,
			    chip->dfs_root, chip, &force_usb_psy_update_ops);
	if (IS_ERR_OR_NULL(file))
		qpnp_err("Couldn't create force_usb_psy_update file rc=%ld\n",
			(long)file);

	file = debugfs_create_file("force_dc_psy_update", S_IRUSR | S_IWUSR,
			    chip->dfs_root, chip, &force_dc_psy_update_ops);
	if (IS_ERR_OR_NULL(file))
		qpnp_err("Couldn't create force_dc_psy_update file rc=%ld\n",
			(long)file);
}
...
static int smb2_probe(struct platform_device *pdev)
{
	...
	if (!chg->regmap) {
		qpnp_err("parent regmap is missing\n");
		return -EINVAL;
	}
	...
	if (rc < 0) {
		if (rc != -EPROBE_DEFER)
			qpnp_err("Couldn't setup chg_config rc=%d\n", rc);
		return rc;
	}

	rc = smb2_parse_dt(chip);
	if (rc < 0) {
		qpnp_err("Couldn't parse device tree rc=%d\n", rc);
		goto cleanup;
	}

	rc = smblib_init(chg);
	if (rc < 0) {
		qpnp_err("Smblib_init failed rc=%d\n", rc);
		goto cleanup;
	}
	...
	if (rc < 0) {
		qpnp_err("Couldn't initialize vbus regulator rc=%d\n",
			rc);
		goto cleanup;
	}
	...
	if (rc < 0) {
		qpnp_err("Couldn't initialize vconn regulator rc=%d\n",
				rc);
		goto cleanup;
	}
	...
	if (rc < 0) {
		qpnp_err("Couldn't initialize hardware rc=%d\n", rc);
		goto cleanup;
	}

	rc = smb2_init_dc_psy(chip);
	if (rc < 0) {
		qpnp_err("Couldn't initialize dc psy rc=%d\n", rc);
		goto cleanup;
	}

	rc = smb2_init_usb_psy(chip);
	if (rc < 0) {
		qpnp_err("Couldn't initialize usb psy rc=%d\n", rc);
		goto cleanup;
	}

	rc = smb2_init_usb_main_psy(chip);
	if (rc < 0) {
		qpnp_err("Couldn't initialize usb main psy rc=%d\n", rc);
		goto cleanup;
	}

	rc = smb2_init_usb_port_psy(chip);
	if (rc < 0) {
		qpnp_err("Couldn't initialize usb pc_port psy rc=%d\n", rc);
		goto cleanup;
	}

	rc = smb2_init_batt_psy(chip);
	if (rc < 0) {
		qpnp_err("Couldn't initialize batt psy rc=%d\n", rc);
		goto cleanup;
	}
	...
	if (rc < 0) {
		qpnp_err("Couldn't determine initial status rc=%d\n",
			rc);
		goto cleanup;
	}
	...
	if (rc < 0) {
		qpnp_err("Couldn't request interrupts rc=%d\n", rc);
		goto cleanup;
	}

	rc = smb2_post_init(chip);
	if (rc < 0) {
		qpnp_err("Failed in post init rc=%d\n", rc);
		goto cleanup;
	}
	...
	rc = smblib_get_prop_batt_present(chg, &val);
	if (rc < 0) {
		qpnp_err("Couldn't get batt present rc=%d\n", rc);
		goto cleanup;
	}
	...
	rc = smblib_get_prop_batt_health(chg, &val);
	if (rc < 0) {
		qpnp_err("Couldn't get batt health rc=%d\n", rc);
	...
	rc = smblib_get_prop_batt_charge_type(chg, &val);
	if (rc < 0) {
		qpnp_err("Couldn't get batt charge type rc=%d\n", rc);
		goto cleanup;
	}
