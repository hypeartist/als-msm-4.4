				...
				enum fg_sram_param_id id, int value)
{
	...
	fg_debug("id: %d raw value: %x decoded value: %x\n", id, value,
		sp[id].value);
	return sp[id].value;
}
...
{
	if (!sp[id].decode) {
		fg_err("No decoding function for parameter %d\n", id);
		return -EINVAL;
	}
				...
				enum fg_sram_param_id  id, int val_mv, u8 *buf)
{
	...
	fg_debug("temp: %llx id: %d, val_mv: %d, buf: [ ", temp, id, val_mv);
				...
				enum fg_sram_param_id  id, int val_ma, u8 *buf)
{
	...
	fg_debug("temp: %llx id: %d, val: %d, buf: [ ", temp, id, val_ma);
	for (i = 0; i < sp[id].len; i++) {
		buf[i] = temp & mask;
		temp >>= 8;
		fg_debug("%x ", buf[i]);
				...
				enum fg_sram_param_id  id, int val, u8 *buf)
{
	...
	fg_debug("temp: %llx id: %d, val: %d, buf: [ ", temp, id, val);
	for (i = 0; i < sp[id].len; i++) {
		buf[i] = temp & mask;
		temp >>= 8;
		fg_debug("%x ", buf[i]);
...
{
	if (!sp[id].encode) {
		fg_err("No encoding function for parameter %d\n", id);
		return;
	}
				...
				int *val)
{
	...
	if (rc < 0) {
		fg_err("Error reading address 0x%04x[%d] rc=%d\n",
...
static int fg_get_charge_raw(struct fg_chip *chip, int *val)
{
	...
	if (rc < 0) {
		fg_err("Error in getting CC_SOC, rc=%d\n", rc);
		return rc;
	}
...
static int fg_get_charge_counter_shadow(struct fg_chip *chip, int *val)
{
	...
	if (rc < 0) {
		fg_err("Error in getting BATT_SOC, rc=%d\n", rc);
		return rc;
	}
...
static int fg_get_charge_counter(struct fg_chip *chip, int *val)
{
	...
	if (rc < 0) {
		fg_err("Error in getting CC_SOC_SW, rc=%d\n", rc);
		return rc;
	}
				...
				enum jeita_levels level, int *temp_decidegC)
{
	...
	if (rc < 0) {
		fg_err("Error in reading jeita level %d, rc=%d\n", level, rc);
		return rc;
	}
...
static int fg_get_battery_temp(struct fg_chip *chip, int *val)
{
	...
	if (rc < 0) {
		fg_err("failed to read addr=0x%04x, rc=%d\n",
			BATT_INFO_BATT_TEMP_LSB(chip), rc);
		return rc;
	}
		fg_err("addr=0x%04x,buf1=%04x buf0=%04x\n",
...
static int fg_get_battery_resistance(struct fg_chip *chip, int *val)
{
	...
	if (rc < 0) {
		fg_err("failed to get ESR, rc=%d\n", rc);
		return rc;
	}

	rc = fg_get_sram_prop(chip, FG_SRAM_RSLOW, &rslow_uohms);
	if (rc < 0) {
		fg_err("failed to get Rslow, rc=%d\n", rc);
		return rc;
	}
...
static int fg_get_msoc_raw(struct fg_chip *chip, int *val)
{
	...
	if (tries == MAX_TRIES_SOC) {
		fg_err("shadow registers do not match\n");
		return -EINVAL;
	}
...
static bool is_batt_empty(struct fg_chip *chip)
{
	...
	if (rc < 0) {
		fg_err("failed to get battery voltage, rc=%d\n", rc);
		return false;
	}
...
static int fg_get_debug_batt_id(struct fg_chip *chip, int *batt_id)
{
	...
	do_div(temp, 150 * 1024);
	batt_id[1] = temp;
	fg_debug("debug batt_id range: [%d %d]\n", batt_id[0], batt_id[1]);
	return 0;
}
...
static bool is_debug_batt_id(struct fg_chip *chip)
{
	...
	if (rc < 0) {
		fg_err("Failed to get debug batt_id, rc=%d\n", rc);
		return false;
	}
...
static int fg_batt_missing_config(struct fg_chip *chip, bool enable)
{
	...
	if (rc < 0)
		fg_err("Error in writing to %04x, rc=%d\n",
...
static int fg_get_batt_id(struct fg_chip *chip)
{
	...
	if (rc < 0) {
		fg_err("Error in disabling BMD, rc=%d\n", rc);
		return rc;
	}

	rc = iio_read_channel_processed(chip->batt_id_chan, &batt_id);
	if (rc < 0) {
		fg_err("Error in reading batt_id channel, rc:%d\n", rc);
		goto out;
	}
...
out:
	...
	if (ret < 0) {
		fg_err("Error in enabling BMD, ret=%d\n", ret);
		return ret;
	}
...
static int fg_get_batt_profile(struct fg_chip *chip)
{
	...
	if (!batt_node) {
		fg_err("Batterydata not available\n");
		return -ENXIO;
	}
	...
	if (!profile_node) {
		fg_err("couldn't find profile handle\n");
		return -ENODATA;
	}
	...
	if (rc < 0) {
		fg_err("battery type unavailable, rc:%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		fg_err("battery float voltage unavailable, rc:%d\n", rc);
		chip->bp.float_volt_uv = -EINVAL;
	}

	if (hwc_check_global){
		fg_err("sunxing get global set fastchg  2.3A");
...
#endif
		if (rc < 0) {
			fg_err("battery fastchg current unavailable, rc:%d\n", rc);
	...
	if (rc < 0) {
		fg_err("battery cc_cv threshold unavailable, rc:%d\n", rc);
		chip->bp.vbatt_full_mv = -EINVAL;
	}

	data = of_get_property(profile_node, "qcom,fg-profile-data", &len);
	if (!data) {
		fg_err("No profile data available\n");
		return -ENODATA;
	}
	...
	if (len != PROFILE_LEN) {
		fg_err("battery profile incorrect size: %d\n", len);
		return -EINVAL;
	}
				...
				int cycles_max, bool charging, int flags)
{
	...
	if (rc < 0) {
		fg_err("Error in writing esr_timer_dischg_max, rc=%d\n",
			rc);
		return rc;
	}
	...
	if (rc < 0) {
		fg_err("Error in writing esr_timer_dischg_init, rc=%d\n",
			rc);
		return rc;
	}
...
static void fg_notify_charger(struct fg_chip *chip)
{
	...
	if (rc < 0) {
		fg_err("Error in setting voltage_max property on batt_psy, rc=%d\n",
			rc);
		return;
	}
	...
	if (rc < 0) {
		fg_err("Error in setting constant_charge_current_max property on batt_psy, rc=%d\n",
			rc);
		return;
	}
...
static int fg_prime_cc_soc_sw(struct fg_chip *chip, int cc_soc_sw)
{
	...
	if (rc < 0)
		fg_err("Error in writing cc_soc_sw, rc=%d\n", rc);
	else
...
static int fg_save_learned_cap_to_sram(struct fg_chip *chip)
{
	...
	if (rc < 0) {
		fg_err("Error in writing act_batt_cap_bkup, rc=%d\n", rc);
		return rc;
	}
	...
	rc = fg_sram_write(chip, ACT_BATT_CAP_WORD, ACT_BATT_CAP_OFFSET,
			...
			FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("Error in writing act_batt_cap, rc=%d\n", rc);
		return rc;
	}
...
static int fg_load_learned_cap_from_sram(struct fg_chip *chip)
{
	...
	if (rc < 0) {
		fg_err("Error in getting ACT_BATT_CAP, rc=%d\n", rc);
		return rc;
	}
	...
	if (chip->cl.learned_cc_uah != chip->cl.nom_cap_uah) {
		...
		if (rc < 0)
			fg_err("Error in saving learned_cc_uah, rc=%d\n", rc);
	}
...
static bool is_temp_valid_cap_learning(struct fg_chip *chip)
{
	...
	if (rc < 0) {
		fg_err("Error in getting batt_temp\n");
		return false;
	}
...
static int fg_cap_learning_done(struct fg_chip *chip)
{
	...
	if (rc < 0) {
		fg_err("Error in processing cap learning full data, rc=%d\n",
			rc);
		goto out;
	}
...
static void fg_cap_learning_update(struct fg_chip *chip)
{
	...
	if (!chip->cl.active) {
		...
		if (chip->charge_done) {
			...
			if (rc < 0)
				fg_err("Error in completing capacity learning, rc=%d\n",
					rc);
...
static int fg_adjust_ki_coeff_dischg(struct fg_chip *chip)
{
	...
	if (rc < 0) {
		fg_err("Error in getting capacity, rc=%d\n", rc);
		return rc;
	}
	...
	rc = fg_sram_write(chip,
			...
			FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("Error in writing ki_coeff_med, rc=%d\n", rc);
		return rc;
	}
	...
	rc = fg_sram_write(chip,
			...
			FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("Error in writing ki_coeff_hi, rc=%d\n", rc);
		return rc;
	}
...
static int fg_adjust_ki_coeff_full_soc(struct fg_chip *chip, int batt_temp)
{
	...
	rc = fg_sram_write(chip,
			...
			FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("Error in writing ki_coeff_full_soc, rc=%d\n", rc);
		return rc;
	}
...
static int fg_set_recharge_voltage(struct fg_chip *chip, int voltage_mv)
{
	...
	rc = fg_sram_write(chip,
			...
			FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("Error in writing recharge_vbatt_thr, rc=%d\n",
			rc);
		return rc;
	}
...
static int fg_configure_full_soc(struct fg_chip *chip, int bsoc)
{
	...
	if (rc < 0) {
		fg_err("failed to write full_soc rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		fg_err("failed to write monotonic_soc rc=%d\n", rc);
		return rc;
	}
...
static int fg_charge_full_update(struct fg_chip *chip)
{
	...
	rc = power_supply_get_property(chip->batt_psy, POWER_SUPPLY_PROP_HEALTH,
		&prop);
	if (rc < 0) {
		fg_err("Error in getting battery health, rc=%d\n", rc);
		goto out;
	}
	...
	if (rc < 0) {
		fg_err("Error in getting msoc_raw, rc=%d\n", rc);
		goto out;
	}
	...
	if (chip->charge_done && !chip->charge_full) {
		if (msoc >= 99 && chip->health == POWER_SUPPLY_HEALTH_GOOD) {
			...
			if (rc < 0) {
				fg_err("Error in reducing recharge voltage, rc=%d\n",
					rc);
				goto out;
			}
		} else {
		...
		if (rc < 0) {
			fg_err("Error in setting recharge voltage, rc=%d\n",
				rc);
			goto out;
		}
...
static int fg_rconn_config(struct fg_chip *chip)
{
	...
	if (rc < 0) {
		fg_err("Error in reading SW_CONFIG_OFFSET, rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		fg_err("Error in reading ESR_RSLOW_CHG_OFFSET, rc=%d\n", rc);
		return rc;
	}
	...
	rc = fg_sram_write(chip, ESR_RSLOW_CHG_WORD,
			ESR_RSLOW_CHG_OFFSET, (u8 *)&val, 1, FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("Error in writing ESR_RSLOW_CHG_OFFSET, rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		fg_err("Error in reading ESR_RSLOW_DISCHG_OFFSET, rc=%d\n", rc);
		return rc;
	}
	...
	rc = fg_sram_write(chip, ESR_RSLOW_DISCHG_WORD,
			ESR_RSLOW_DISCHG_OFFSET, (u8 *)&val, 1, FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("Error in writing ESR_RSLOW_DISCHG_OFFSET, rc=%d\n", rc);
		return rc;
	}
	...
	rc = fg_sram_write(chip, PROFILE_INTEGRITY_WORD,
			SW_CONFIG_OFFSET, (u8 *)&val, 1, FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("Error in writing SW_CONFIG_OFFSET, rc=%d\n", rc);
		return rc;
	}
				...
				enum jeita_levels level, int temp_decidegC)
{
	...
	if (rc < 0) {
		fg_err("Error in setting jeita level %d, rc=%d\n", level, rc);
		return rc;
	}
...
static int fg_set_constant_chg_voltage(struct fg_chip *chip, int volt_uv)
{
	...
	if (rc < 0) {
		fg_err("Error in writing vbatt_full, rc=%d\n", rc);
		return rc;
	}
...
static int fg_set_recharge_soc(struct fg_chip *chip, int recharge_soc)
{
	...
	if (rc < 0) {
		fg_err("Error in writing recharge_soc_thr, rc=%d\n", rc);
		return rc;
	}
...
static int fg_adjust_recharge_soc(struct fg_chip *chip)
{
	...
	if (is_input_present(chip)) {
		if (chip->charge_done) {
			if (!chip->recharge_soc_adjusted) {
				...
				rc = fg_get_msoc(chip, &msoc);
				if (rc < 0) {
					fg_err("Error in getting msoc, rc=%d\n",
						rc);
					return rc;
				}
	...
	if (rc < 0) {
		...
		fg_err("Couldn't set resume SOC for FG, rc=%d\n", rc);
		return rc;
	}
...
if (chip->health == POWER_SUPPLY_HEALTH_COOL)
	...
	if (rc < 0) {
		fg_err("Error in setting recharge_voltage, rc=%d\n",
			rc);
		return rc;
	}
...
static int fg_slope_limit_config(struct fg_chip *chip, int batt_temp)
{
	...
	if (rc < 0) {
		fg_err("Error in configuring slope_limit coefficient, rc=%d\n",
			rc);
		return rc;
	}
				...
				enum esr_filter_status esr_flt_sts)
{
	...
	if (esr_flt_sts == ROOM_TEMP) {
		...
		fg_err("Unknown esr filter config\n");
		return 0;
	}
	...
	if (rc < 0) {
		fg_err("Error in writing ESR LT tight filter, rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		fg_err("Error in writing ESR LT broad filter, rc=%d\n", rc);
		return rc;
	}
				...
				bool override)
{
	...
	if (qnovo_en && !override) {
		if (input_present) {
			...
			if (rc < 0)
				fg_err("Couldn't cancel esr_filter_alarm\n");
		}
	}
...
static void esr_filter_work(struct work_struct *work)
{
	...
	if (rc < 0) {
		fg_err("Error in configuring ESR filter rc:%d\n", rc);
		alarm_start_relative(&chip->esr_filter_alarm,
			ms_to_ktime(FG_ESR_FILTER_RESTART_MS));
	}
...
static int fg_esr_fcc_config(struct fg_chip *chip)
{
	...
	if (is_parallel_charger_available(chip)) {
		...
		if (rc < 0) {
			fg_err("Error in reading charging_enabled from parallel_psy, rc=%d\n",
				rc);
			return rc;
		}
...
static int fg_esr_timer_config(struct fg_chip *chip, bool sleep)
{
	...
	if (rc < 0) {
		fg_err("Error in setting ESR timer, rc=%d\n", rc);
		return rc;
	}
...
static void fg_ttf_update(struct fg_chip *chip)
{
	...
	if (usb_psy_initialized(chip)) {
		...
		if (rc < 0) {
			fg_err("Couldn't read usb ONLINE prop rc=%d\n", rc);
			return;
		}
	...
	if (pc_port_psy_initialized(chip)) {
		rc = power_supply_get_property(chip->pc_port_psy,
			POWER_SUPPLY_PROP_ONLINE, &prop);
		if (rc < 0) {
			fg_err("Couldn't read pc_port ONLINE prop rc=%d\n", rc);
			return;
		}
	...
	if (dc_psy_initialized(chip)) {
		rc = power_supply_get_property(chip->dc_psy,
			POWER_SUPPLY_PROP_ONLINE, &prop);
		if (rc < 0) {
			fg_err("Couldn't read dc ONLINE prop rc=%d\n", rc);
			return;
		}
...
static void restore_cycle_counter(struct fg_chip *chip)
{
	...
	for (i = 0; i < BUCKET_COUNT; i++) {
		rc = fg_sram_read(chip, CYCLE_COUNT_WORD + (i / 2),
				...
				FG_IMA_DEFAULT);
		if (rc < 0)
			fg_err("failed to read bucket %d rc=%d\n", i, rc);
		else
...
static void clear_cycle_counter(struct fg_chip *chip)
{
	...
	rc = fg_sram_write(chip, CYCLE_COUNT_WORD, CYCLE_COUNT_OFFSET,
			...
			FG_IMA_DEFAULT);
	if (rc < 0)
		fg_err("failed to clear cycle counter rc=%d\n", rc);
...
static int fg_inc_store_cycle_ctr(struct fg_chip *chip, int bucket)
{
	...
	rc = fg_sram_write(chip, CYCLE_COUNT_WORD + (bucket / 2),
			...
			FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("failed to write BATT_CYCLE[%d] rc=%d\n",
			bucket, rc);
		return rc;
	}
...
static void fg_cycle_counter_update(struct fg_chip *chip)
{
	...
	rc = fg_get_sram_prop(chip, FG_SRAM_BATT_SOC, &batt_soc);
	if (rc < 0) {
		fg_err("Failed to read battery soc rc: %d\n", rc);
		goto out;
	}
	...
	if (chip->charge_status == POWER_SUPPLY_STATUS_CHARGING) {
		...
		for (i = 0; i < BUCKET_COUNT; i++) {
			if (chip->cyc_ctr.started[i] &&
				...
				if (rc < 0)
					fg_err("Error in storing cycle_ctr rc: %d\n",
						rc);
				chip->cyc_ctr.last_soc[i] = 0;
				chip->cyc_ctr.started[i] = false;
			}
...
static void status_change_work(struct work_struct *work)
{
	...
	rc = power_supply_get_property(chip->batt_psy, POWER_SUPPLY_PROP_STATUS,
			&prop);
	if (rc < 0) {
		fg_err("Error in getting charging status, rc=%d\n", rc);
		goto out;
	}
	...
	if (rc < 0) {
		fg_err("Error in getting charge type, rc=%d\n", rc);
		goto out;
	}
	...
	if (rc < 0) {
		fg_err("Error in getting charge_done, rc=%d\n", rc);
		goto out;
	}
	...
	if (rc < 0)
		fg_err("Error in charge_full_update, rc=%d\n", rc);

	rc = fg_adjust_recharge_soc(chip);
	if (rc < 0)
		fg_err("Error in adjusting recharge_soc, rc=%d\n", rc);

	rc = fg_adjust_recharge_voltage(chip);
	if (rc < 0)
		fg_err("Error in adjusting recharge_voltage, rc=%d\n", rc);

	rc = fg_adjust_ki_coeff_dischg(chip);
	if (rc < 0)
		fg_err("Error in adjusting ki_coeff_dischg, rc=%d\n", rc);

	rc = fg_esr_fcc_config(chip);
	if (rc < 0)
		fg_err("Error in adjusting FCC for ESR, rc=%d\n", rc);

	rc = fg_get_battery_temp(chip, &batt_temp);
	if (!rc) {
		rc = fg_slope_limit_config(chip, batt_temp);
		if (rc < 0)
			fg_err("Error in configuring slope limiter rc:%d\n",
				rc);

		rc = fg_adjust_ki_coeff_full_soc(chip, batt_temp);
		if (rc < 0)
			fg_err("Error in configuring ki_coeff_full_soc rc:%d\n",
				rc);
	}
...
static int fg_bp_params_config(struct fg_chip *chip)
{
	...
	if (!(chip->wa_flags & PMI8998_V1_REV_WA) &&
		...
		if (rc < 0) {
			fg_err("Error in writing float_volt, rc=%d\n", rc);
			return rc;
		}
	}
...
static bool is_profile_load_required(struct fg_chip *chip)
{
	...
	if (rc < 0) {
		fg_err("failed to read profile integrity rc=%d\n", rc);
		return false;
	}
	...
	if (val & PROFILE_LOAD_BIT) {
		...
		if (rc < 0) {
			fg_err("Error in reading battery profile, rc:%d\n", rc);
			return false;
		}
...
static void clear_battery_profile(struct fg_chip *chip)
{
	...
	rc = fg_sram_write(chip, PROFILE_INTEGRITY_WORD,
			PROFILE_INTEGRITY_OFFSET, &val, 1, FG_IMA_DEFAULT);
	if (rc < 0)
		fg_err("failed to write profile integrity rc=%d\n", rc);
}
...
wait:
	...
	if (rc == -ERESTARTSYS && !tried_again) {
		...
		fg_err("wait for soc_ready timed out rc=%d\n", rc);
	}
...
static void profile_load_work(struct work_struct *work)
{
	...
	if (rc < 0) {
		fg_err("Error in getting battery id, rc:%d\n", rc);
		goto out;
	}
	...
	if (rc < 0) {
		fg_err("Error in writing battery profile, rc:%d\n", rc);
		goto out;
	}

	rc = __fg_restart(chip);
	if (rc < 0) {
		fg_err("Error in restarting FG, rc=%d\n", rc);
		goto out;
	}
	...
	if (rc < 0)
		fg_err("Error in configuring battery profile params, rc:%d\n",
			rc);

	rc = fg_sram_read(chip, NOM_CAP_WORD, NOM_CAP_OFFSET, buf, 2,
			FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("Error in reading %04x[%d] rc=%d\n", NOM_CAP_WORD,
			NOM_CAP_OFFSET, rc);
	} else {
		chip->cl.nom_cap_uah = (int)(buf[0] | buf[1] << 8) * 1000;
		rc = fg_load_learned_cap_from_sram(chip);
		if (rc < 0)
			fg_err("Error in loading capacity learning data, rc:%d\n",
				rc);
	}

	rc = fg_rconn_config(chip);
	if (rc < 0)
		fg_err("Error in configuring Rconn, rc=%d\n", rc);
...
static void sram_dump_work(struct work_struct *work)
{
	...
	if (rc < 0) {
		fg_err("Error in reading FG SRAM, rc:%d\n", rc);
		goto resched;
	}
...
static int fg_sram_dump_sysfs(const char *val, const struct kernel_param *kp)
{
	...
	if (rc) {
		fg_err("Unable to set fg_sram_dump: %d\n", rc);
		return rc;
	}
...
static int fg_restart_sysfs(const char *val, const struct kernel_param *kp)
{
	...
	if (rc) {
		fg_err("Unable to set fg_restart: %d\n", rc);
		return rc;
	}
...
static int fg_get_time_to_full_locked(struct fg_chip *chip, int *val)
{
	...
	if (chip->bp.float_volt_uv <= 0) {
		fg_err("battery profile is not loaded\n");
		return -ENODATA;
	}
	...
	rc = fg_get_prop_capacity(chip, &msoc);
	if (rc < 0) {
		fg_err("failed to get msoc rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		fg_err("failed to get IBATT AVG rc=%d\n", rc);
		return rc;
	}

	rc = fg_circ_buf_median(&chip->ttf.vbatt, &vbatt_avg);
	if (rc < 0) {
		fg_err("failed to get VBATT AVG rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		fg_err("failed to get battery resistance rc=%d\n", rc);
		return rc;
	}
	...
	rc = fg_get_sram_prop(chip, FG_SRAM_ACT_BATT_CAP, &act_cap_mah);
	if (rc < 0) {
		fg_err("failed to get ACT_BATT_CAP rc=%d\n", rc);
		return rc;
	}

	rc = fg_get_sram_prop(chip, FG_SRAM_FULL_SOC, &full_soc);
	if (rc < 0) {
		fg_err("failed to get full soc rc=%d\n", rc);
		return rc;
	}
	...
	case TTF_MODE_QNOVO:
		...
		break;
	default:
		fg_err("TTF mode %d is not supported\n", chip->ttf.mode);
		break;
	}
...
cv_estimate:
	...
	if (rc < 0) {
		fg_err("failed to interpolate tau rc=%d\n", rc);
		return rc;
	}
...
static int fg_get_time_to_empty(struct fg_chip *chip, int *val)
{
	...
	if (rc < 0) {
		...
		if (rc < 0) {
			fg_err("failed to get battery current, rc=%d\n", rc);
			return rc;
		}
	}
...
static int fg_esr_validate(struct fg_chip *chip)
{
	...
	if (rc < 0) {
		fg_err("Error in writing ESR, rc=%d\n", rc);
		return rc;
	}
...
static int fg_force_esr_meas(struct fg_chip *chip)
{
	...
	rc = fg_sram_masked_write(chip, ESR_EXTRACTION_ENABLE_WORD,
			...
			FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("failed to enable esr extn rc=%d\n", rc);
		goto out;
	}
	...
	rc = fg_masked_write(chip, BATT_INFO_QNOVO_CFG(chip),
			LD_REG_CTRL_BIT, 0);
	if (rc < 0) {
		fg_err("Error in configuring qnovo_cfg rc=%d\n", rc);
		goto out;
	}
	...
	rc = fg_masked_write(chip, BATT_INFO_TM_MISC1(chip),
			...
			ESR_REQ_CTL_BIT | ESR_REQ_CTL_EN_BIT);
	if (rc < 0) {
		fg_err("Error in configuring force ESR rc=%d\n", rc);
		goto out;
	}
	...
	rc = fg_masked_write(chip, BATT_INFO_TM_MISC1(chip),
			...
			0);
	if (rc < 0) {
		fg_err("Error in restoring force ESR rc=%d\n", rc);
		goto out;
	}
	...
	if (rc < 0) {
		fg_err("Error in restoring qnovo_cfg rc=%d\n", rc);
		goto out;
	}
	...
	rc = fg_sram_masked_write(chip, ESR_EXTRACTION_ENABLE_WORD,
			...
			FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("failed to disable esr extn rc=%d\n", rc);
		goto out;
	}
...
static int fg_prepare_for_qnovo(struct fg_chip *chip, int qnovo_enable)
{
	...
	rc = fg_sram_masked_write(chip, ESR_EXTRACTION_ENABLE_WORD,
			...
			FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("Error in configuring esr extraction rc=%d\n", rc);
		goto out;
	}
				       ...
				       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_COLD_TEMP:
		...
		if (rc < 0) {
			fg_err("Error in reading jeita_cold, rc=%d\n", rc);
			return rc;
		}
		break;
	case POWER_SUPPLY_PROP_COOL_TEMP:
		...
		if (rc < 0) {
			fg_err("Error in reading jeita_cool, rc=%d\n", rc);
			return rc;
		}
		break;
	case POWER_SUPPLY_PROP_WARM_TEMP:
		...
		if (rc < 0) {
			fg_err("Error in reading jeita_warm, rc=%d\n", rc);
			return rc;
		}
		break;
	case POWER_SUPPLY_PROP_HOT_TEMP:
		...
		if (rc < 0) {
			fg_err("Error in reading jeita_hot, rc=%d\n", rc);
			return rc;
		}
		break;
	...
	case POWER_SUPPLY_PROP_CC_STEP:
		if ((chip->ttf.cc_step.sel >= 0) &&
			...
			fg_err("cc_step_sel is out of bounds [0, %d]\n",
				  ...
				  const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CYCLE_COUNT_ID:
		if ((pval->intval > 0) && (pval->intval <= BUCKET_COUNT)) {
			...
			fg_err("rejecting invalid cycle_count_id = %d\n",
				pval->intval);
			return -EINVAL;
		}
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		...
		if (pval->intval <= 0 || pval->intval > chip->cl.nom_cap_uah) {
			fg_err("charge_full is out of bounds\n");
			return -EINVAL;
		}
	...
	case POWER_SUPPLY_PROP_COLD_TEMP:
		...
		if (rc < 0) {
			fg_err("Error in writing jeita_cold, rc=%d\n", rc);
			return rc;
		}
		break;
	case POWER_SUPPLY_PROP_COOL_TEMP:
		...
		if (rc < 0) {
			fg_err("Error in writing jeita_cool, rc=%d\n", rc);
			return rc;
		}
		break;
	case POWER_SUPPLY_PROP_WARM_TEMP:
		...
		if (rc < 0) {
			fg_err("Error in writing jeita_warm, rc=%d\n", rc);
			return rc;
		}
		break;
	case POWER_SUPPLY_PROP_HOT_TEMP:
		...
		if (rc < 0) {
			fg_err("Error in writing jeita_hot, rc=%d\n", rc);
			return rc;
		}
		break;
	default:
...
static int fg_hw_init(struct fg_chip *chip)
{
	...
	if (rc < 0) {
		fg_err("Error in writing cutoff_volt, rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		fg_err("Error in writing empty_volt, rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		fg_err("Error in writing chg_term_curr, rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		fg_err("Error in writing sys_term_curr, rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		fg_err("Error in writing cutoff_curr, rc=%d\n", rc);
		return rc;
	}
	...
	if (!(chip->wa_flags & PMI8998_V1_REV_WA)) {
		...
		rc = fg_sram_write(chip,
				...
				FG_IMA_DEFAULT);
		if (rc < 0) {
			fg_err("Error in writing chg_term_base_curr, rc=%d\n",
				rc);
			return rc;
		}
	}
	...
	if (chip->dt.vbatt_low_thr_mv > 0) {
		...
		rc = fg_sram_write(chip, chip->sp[FG_SRAM_VBATT_LOW].addr_word,
				...
				FG_IMA_DEFAULT);
		if (rc < 0) {
			fg_err("Error in writing vbatt_low_thr, rc=%d\n", rc);
			return rc;
		}
	}
	...
	if (chip->dt.delta_soc_thr > 0 && chip->dt.delta_soc_thr < 100) {
		...
		rc = fg_sram_write(chip,
				...
				FG_IMA_DEFAULT);
		if (rc < 0) {
			fg_err("Error in writing delta_msoc_thr, rc=%d\n", rc);
			return rc;
		}
		...
		rc = fg_sram_write(chip,
				...
				FG_IMA_DEFAULT);
		if (rc < 0) {
			fg_err("Error in writing delta_bsoc_thr, rc=%d\n", rc);
			return rc;
		}
	}
	...
	if (chip->dt.batt_therm_coeffs[0] > 0) {
		...
		if (rc < 0) {
			fg_err("Error in writing battery thermal coefficients, rc=%d\n",
				rc);
			return rc;
		}
	}
	...
	if (chip->dt.recharge_soc_thr > 0 && chip->dt.recharge_soc_thr < 100) {
		...
		if (rc < 0) {
			fg_err("Error in setting recharge_soc, rc=%d\n", rc);
			return rc;
		}
	}
	...
	if (chip->dt.rsense_sel >= SRC_SEL_BATFET &&
		...
		if (rc < 0) {
			fg_err("Error in writing rsense_sel, rc=%d\n", rc);
			return rc;
		}
	}
	...
	if (chip->dt.jeita_hyst_temp >= 0) {
		...
		if (rc < 0) {
			fg_err("Error in writing batt_temp_cfg, rc=%d\n", rc);
			return rc;
		}
	}
	...
	if (rc < 0) {
		fg_err("Error in writing batt_temp_delta, rc=%d\n", rc);
		return rc;
	}
	...
	rc = fg_sram_write(chip, chip->sp[FG_SRAM_ESR_TIGHT_FILTER].addr_word,
			...
			chip->sp[FG_SRAM_ESR_TIGHT_FILTER].len, FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("Error in writing ESR tight filter, rc=%d\n", rc);
		return rc;
	}
	...
	rc = fg_sram_write(chip, chip->sp[FG_SRAM_ESR_BROAD_FILTER].addr_word,
			...
			chip->sp[FG_SRAM_ESR_BROAD_FILTER].len, FG_IMA_DEFAULT);
	if (rc < 0) {
		fg_err("Error in writing ESR broad filter, rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		fg_err("Error in writing esr_pulse_thresh_ma, rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		fg_err("Error in writing esr_meas_curr_ma, rc=%d\n", rc);
		return rc;
	}
	...
	if (is_debug_batt_id(chip)) {
		...
		if (rc < 0) {
			fg_err("Error in writing esr_pull_down, rc=%d\n", rc);
			return rc;
		}
	}
...
static int fg_adjust_timebase(struct fg_chip *chip)
{
	...
	if ((chip->wa_flags & PM660_TSMC_OSC_WA) && chip->die_temp_chan) {
		...
		if (rc < 0) {
			fg_err("Error in reading die_temp, rc:%d\n", rc);
			return rc;
		}
		...
		if (rc < 0) {
			fg_err("Error to lookup fg_tsmc_osc_table rc=%d\n", rc);
			return rc;
		}
		...
		if (rc < 0) {
			fg_err("Error in writing timebase, rc=%d\n", rc);
			return rc;
		}
	}
...
static irqreturn_t fg_mem_xcp_irq_handler(int irq, void *data)
{
	...
	if (rc < 0)
		fg_err("Error in clearing DMA error, rc=%d\n", rc);

	if (status & MEM_XCP_BIT) {
		rc = fg_clear_ima_errors_if_any(chip, true);
		if (rc < 0 && rc != -EAGAIN)
			fg_err("Error in checking IMA errors rc:%d\n", rc);
	}
...
static irqreturn_t fg_delta_batt_temp_irq_handler(int irq, void *data)
{
	...
	if (chip->last_batt_temp != batt_temp) {
		...
		if (rc < 0)
			fg_err("Error in adjusting timebase, rc=%d\n", rc);
...
static irqreturn_t fg_delta_msoc_irq_handler(int irq, void *data)
{
	...
	if (rc < 0)
		fg_err("Error in updating maint_soc, rc=%d\n", rc);

	rc = fg_esr_validate(chip);
	if (rc < 0)
		fg_err("Error in validating ESR, rc=%d\n", rc);
	...
	if (!rc)
		fg_err("lct battery SOC:%d voltage:%duV current:%duA temp:%d id:%dK charge_status:%d charge_type:%d health:%d input_present:%d \n",
				...
				const char *prop_name, int *buf, int len)
{
	...
	if (rc < 0) {
		...
		fg_err("Incorrect length %d for %s, rc=%d\n", len, prop_name,
			rc);
		return -EINVAL;
	}
	...
	if (rc < 0) {
		fg_err("Error in reading %s, rc=%d\n", prop_name, rc);
		return rc;
	}
...
static int fg_parse_slope_limit_coefficients(struct fg_chip *chip)
{
	...
	for (i = 0; i < SLOPE_LIMIT_NUM_COEFFS; i++) {
		if (chip->dt.slope_limit_coeffs[i] > SLOPE_LIMIT_COEFF_MAX ||
			...
			fg_err("Incorrect slope limit coefficient\n");
			return -EINVAL;
		}
	}
...
static int fg_parse_ki_coefficients(struct fg_chip *chip)
{
	...
	for (i = 0; i < KI_COEFF_SOC_LEVELS; i++) {
		if (chip->dt.ki_coeff_soc[i] < 0 ||
			...
			fg_err("Error in ki_coeff_soc_dischg values\n");
			return -EINVAL;
		}

		if (chip->dt.ki_coeff_med_dischg[i] < 0 ||
			chip->dt.ki_coeff_med_dischg[i] > KI_COEFF_MAX) {
			fg_err("Error in ki_coeff_med_dischg values\n");
			return -EINVAL;
		}
...
static int fg_parse_dt(struct fg_chip *chip)
{
	...
	if (!revid_node) {
		fg_err("Missing qcom,pmic-revid property - driver failed\n");
		return -EINVAL;
	}

	chip->pmic_rev_id = get_revid_data(revid_node);
	if (IS_ERR_OR_NULL(chip->pmic_rev_id)) {
		fg_err("Unable to get pmic_revid rc=%ld\n",
	...
	fg_debug("PMIC subtype %d Digital major %d\n",
	...
	if (rc < 0)
		fg_err("Error in parsing Ki coefficients, rc=%d\n", rc);
	...
	if (rc < 0)
		fg_err("Error in parsing slope limit coeffs, rc=%d\n", rc);
...
static int fg_gen3_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(chip->batt_id_chan)) {
		if (PTR_ERR(chip->batt_id_chan) != -EPROBE_DEFER)
			fg_err("batt_id_chan unavailable %ld\n",
	...
	if (rc >= 0) {
		...
		if (IS_ERR(chip->die_temp_chan)) {
			if (PTR_ERR(chip->die_temp_chan) != -EPROBE_DEFER)
				fg_err("rradc_die_temp unavailable %ld\n",
	...
	if (IS_ERR(chip->fg_psy)) {
		fg_err("failed to register fg_psy rc = %ld\n",
				PTR_ERR(chip->fg_psy));
		goto exit;
	}
	...
	fg_debug("FG GEN3 driver probed successfully\n");
	return 0;
exit:
	fg_cleanup(chip);
	return rc;
...
static int fg_gen3_suspend(struct device *dev)
{
	...
	if (rc < 0)
		fg_err("Error in configuring ESR timer, rc=%d\n", rc);
...
static void fg_gen3_shutdown(struct platform_device *pdev)
{
	...
	if (chip->charge_full) {
		...
		rc = fg_configure_full_soc(chip, bsoc);
		if (rc < 0) {
			fg_err("Error in configuring full_soc, rc=%d\n", rc);
			return;
		}
	}
}
